...
let fullName = [];
let result = runners.forEach(runner => {
...
let ticketPriceTotal = runners.reduce((prev, curr) => {
    return prev + curr.donation;
}, 0);
...
// Problem 1
//The drunk director dislikes the '@' symbol and would prefer the word 'CAT' to be exchanged in its place.
let catSymbol = runners.map(runner => {
    return runner.email
        .split('')
        .map(char => {
            return char === '@' ? (char = 'CAT') : char;
        })
        .join('');
});
console.log(catSymbol);
// Problem 2
//Administration would like to have a list of first name, last name, company name and amount of all participants who donated more than $250 sorted from greatest to least amount.
let largeDonors = runners
    .filter(runner => runner.donation > 250)
    .sort((a, b) => (a.donation < b.donation ? 1 : -1))
    .map(person => {
        return `$${person.donation} ${person.first_name} ${person.last_name}`;
    });
console.log(largeDonors);

// Problem 3
//Administration needs a count of each shirt size to know how many of each shirt they need to order
const shirtCount = runners.reduce((prev, curr) => {
    prev[curr.shirt_size] = (prev[curr.shirt_size] || 0) + 1;
    return prev;
}, {});
